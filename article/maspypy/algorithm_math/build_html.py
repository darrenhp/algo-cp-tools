# -*- coding: utf-8 -*-
"""Convert a combined Chinese markdown -> styled HTML with sidebar TOC + MathJax.
Usage: python3 build_html.py <src.md> <out.html> <title>
"""
import sys, re
import mistune
...
SRC = sys.argv[1]
OUT = sys.argv[2]
TITLE = sys.argv[3] if len(sys.argv) > 3 else "讲解（maspy 博客中文翻译）"
...
def _wrap_callout(m):
    ...
    elif re.match(r"\s*<strong>【性质", inner):
        cls = "callout-theorem"
    elif re.match(r"\s*<strong>【补题", inner):
        cls = "callout-theorem"
    elif re.match(r"\s*<strong>【命題", inner):
        cls = "callout-theorem"
    elif re.match(r"\s*<strong>【例", inner):
        cls = "callout-definition"
    elif re.match(r"\s*<strong>【証明", inner) or re.match(r"\s*<strong>【证明", inner):
        cls = "callout-solution"
    else:
        return m.group(0)
...
(function(){
  ...
  heads.forEach(function(h){
    ...
    if(h.tagName==='H2'){ root.appendChild(li); cur2=li; cur3=null; }
      ...
      var parent=cur3||cur2; if(!parent) parent=root;
